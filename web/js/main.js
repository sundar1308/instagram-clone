...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function handleSubmit(event) {
      ...
      if(img_container.classList.contains('hide_img')){
          ...
          const imageFile = document.getElementById('image-upload').files[0]; // This line may fail if element doesn't exist.
...
function completed(){
  ...
  share_btn_post.addEventListener("click", function(){
    ...
    $.ajax({
        url: form.attr('action'), // The URL of your action
        type: 'POST',
        data: formData,
        ...
        success: function(response) {
            ...
            if (response.success) {
              ...
              share_btn_post.innerHTML = ""
                ...
                $(this).closest('#post-form').trigger('reset');
                setTimeout(function() {
...
$('#create_modal .btn-close').click(function(){
  $('.post_published').addClass('hide_img');
  $(this).closest('#post-form').trigger('reset');
  $('#create_modal').modal('hide');
});
}
